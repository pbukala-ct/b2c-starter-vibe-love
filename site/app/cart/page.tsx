...
export default function CartPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 lg:px-8 py-8">
      <h1 className="text-2xl font-semibold text-charcoal mb-8">Shopping Cart</h1>
          ...
          <div className="bg-cream p-6 rounded-sm sticky top-24">
            ...
            <div className="space-y-2 text-sm mb-4">
              <div className="flex justify-between">
                <span className="text-charcoal-light">Subtotal ({cart.lineItems.reduce((s, i) => s + i.quantity, 0)} items)</span>
                <span>{formatMoney(cart.totalPrice.centAmount, cart.totalPrice.currencyCode)}</span>
              </div>
              {taxAmount > 0 && (
                <div className="flex justify-between">
                  <span className="text-charcoal-light">{taxIsEstimate ? 'Estimated Tax (10%)' : 'Tax'}</span>
            ...
            {hasSubscriptions && (
              <div className="bg-sage/10 border border-sage/20 rounded-sm p-3 mb-4">
                <p className="text-xs text-charcoal">
                  ♻ Your cart includes subscription items. After checkout, recurring deliveries will be set up automatically.
                </p>
              </div>
            )}
